...
class _InstallRequirementBackedCandidate(Candidate):
    ...
    def iter_dependencies(self, with_requires: bool) -> Iterable[Optional[Requirement]]:
        variant_desc, variant_label = get_variant_description_for_link(self._link)
        requires = self.dist.iter_dependencies(variant_desc=variant_desc, variant_label=variant_label) if with_requires else ()
        for r in requires:
            yield from self._factory.make_requirements_from_spec(str(r), self._ireq, variant_desc=variant_desc, variant_label=variant_label)
